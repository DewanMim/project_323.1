...
<p align="center">
  ...
  <img alt="Build Tool" src="https://img.shields.io/badge/Build-Maven-7A3E00?style=for-the-badge&logo=apachemaven&logoColor=white">
  <img alt="UI" src="https://img.shields.io/badge/UI-Swing-0F766E?style=for-the-badge&logo=java&logoColor=white">
  <img alt="Status" src="https://img.shields.io/badge/Project-Academic%20Simulator-374151?style=for-the-badge">
</p>
...
The GUI is intuitively organized into three clear sections:
1. **Top Control Bar**: Algorithm selection, process actions, and simulation controls.
2. **Center Area**: Input and output tables.
3. **Bottom Panel**: Interactive Gantt chart for timeline visualization.

```mermaid
graph LR
    A[Input Process Table] --> B[Select Algorithm];
    B --> C{Is Round Robin?};
    C -->|Yes| D[Set Time Quantum];
    C -->|No| E[Run Simulation];
    D --> E;
    E --> F[Review Result Table];
    E --> G[Inspect Gantt Chart];
```
...
The application starts with these sample processes so the interface can be explored immediately.
...
*Note: Each process row expects Process ID, Arrival Time, Burst Time, and Priority. Priority is captured by the UI but not currently used by implemented schedulers.*
...
> **Note:** This README reflects the current implementation in the codebase, not planned behavior.
